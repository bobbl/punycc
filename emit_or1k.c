...
/**********************************************************************
 * Step 1: global variables, empty functions and big or little endian
 **********************************************************************/
...
unsigned int emit_begin()
{
    ...
    emit_n(112, "\x7f\x45\x4c\x46\x01\x02\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    "\x00\x02\x00\x5c\x00\x00\x00\x01\x00\x00\x20\x54"
    "\x00\x00\x00\x34\x00\x00\x00\x00\x00\x00\x00\x00"
    "\x00\x34\x00\x20\x00\x01\x00\x00\x00\x00\x00\x00"
    "\x00\x00\x00\x01\x00\x00\x00\x00"
    "\x00\x00\x20\x00\x00\x00\x20\x00........\x00\x00\x00\x07\x00\x00\x10\x00"
    "\x18\x00\x00\x00\x18\x40\x00\x00\xa8\x42\x00\x00\x00\x00\x00\x00\x15\x00\x00\x00\xa9\x60\x00\x5d\x20\x00\x00\x00");
/*
elf_header:
...
program_header_table:
    ...
    003C 00 80 04 08    p_vaddr         0x00002000
    0040 00 80 04 08    p_paddr         0x00002000
    0044 ?? ?? ?? ??    p_filesz
    0048 ?? ?? ?? ??    p_memsz
...
_start:
    ...
    0058 18 40 ?? ??    l.movhi r2, ?   r2 points to the set start of the data section
...
/**********************************************************************
 * Step 4: hardcoded library functions
 **********************************************************************/

unsigned int insn_call(unsigned int disp)
{
    return ((disp >> 2) & 67108863)    | 67108864;
        /*              & 0x03ff'ffff) | 0x0400'0000; */
}
...
void emit_string(unsigned int len, char *s)
{
    ...
    emit_odabi(56, 13, 9, 9, 4);
        /*              l.or r13, r9, r9         # save r9*/
    ...
    emit_odabi(56, reg_pos, 9, 9, 4);
        /*              l.or REG, r9, r9 */
    emit_odabi(56, 9, 13, 13, 4);
        /*              l.or r9, r13, r13         # restore r9*/
...
/**********************************************************************
 * Step 6: read/write global variables
 **********************************************************************/
...
void emit_store(unsigned int global, unsigned int ofs)
{
    if (global) {
        ...
        emit_mv(ofs+13, reg_pos);
            /* l.ori REG[ofs+13], REG[reg_pos], REG[reg_pos] */
    }
}
...
{
    if (global) {
        emit_odabi(53, reg_pos, 2, 0, ofs << 2);
            /* l.lwz REG, OFS(r2) */
    }
    else {
        emit_mv(ofs+13, reg_pos);
            /* l.ori REG[reg_pos], REG[ofs+13], REG[ofs+13] */
    }
}
...
{
    if (global) {
        emit_odabi(53, reg_pos+1, 2, 0, ofs << 2);
            /* l.lwz REG, OFS(r2) */
    }
    else {
        emit_mv(ofs+13, reg_pos+1);
            /* l.ori REG[reg_pos], REG[ofs+13], REG[ofs+13] */
    }
    emit_odabi(53, reg_pos+1, 2, 0, ofs << 2);
...
void emit_index_load_array(unsigned int global, unsigned int ofs)
{
    ...
    emit_odabi(35, reg_pos, reg_pos, 0, ofs << 2);
...
{
    reg_pos = reg_pos - 1;
    char *condition_code = "\x00\x01\x02\x04\x05\x00\x03\x0b\x0a";
...
unsigned int emit_local_var(unsigned int init)
{
    ...
    if (init != 0) {
        emit_mv(num_locals + 13, reg_pos);
            /* l.ori REG[num_locals+13], REG[reg_pos], REG[reg_pos] */
    }
    return num_locals;
}
